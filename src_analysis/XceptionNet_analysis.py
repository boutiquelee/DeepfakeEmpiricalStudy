...
if __name__ == "__main__":
    for i in range(1, 7):
        model_path = f"../model/XceptionNet{i}.h5"
        for dataset in datasets:
            IMAGE_PATH = f"../dataset/{dataset}/test/fake"
            OUTPUT_PATH = f"../heatset/XceptionNet/XceptionNet{i}/{dataset}"
            print(i, model_path, IMAGE_PATH, OUTPUT_PATH)
            
            input_tensor = Input(shape=(256, 256, 3))
            model=tf.keras.applications.Xception(input_tensor=input_tensor,include_top=True, weights=None, classes=2)
